...
import { getOtherHonors } from '../../utils/mockData.js'
...
const loadHonors = () => {
  // 在实际应用中，这里应该从API获取数据
  honors.value = getOtherHonors('all')
}

// 添加新记录
const addRecord = () => {
  // 基本验证
  if (!newRecord.value.name || !newRecord.value.type || !newRecord.value.description) {
    alert('请填写完整信息')
    return
  }
  
  // 在实际应用中，这里应该调用API添加数据
  const newId = Math.max(...honors.value.map(h => h.id), 0) + 1
  honors.value.push({
    id: newId,
    ...newRecord.value,
    avatar: `https://picsum.photos/id/${newId + 150}/64/64`
  })
...
const saveEdit = () => {
  // 基本验证
  if (!editingRecord.value.name || !editingRecord.value.type || !editingRecord.value.description) {
    alert('请填写完整信息')
    return
  }
  
  // 在实际应用中，这里应该调用API更新数据
  const index = honors.value.findIndex(h => h.id === editingRecord.value.id)
  if (index !== -1) {
    honors.value[index] = { ...editingRecord.value }
  }
...
<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-xl font-bold text-gray-800">其他荣誉管理</h2>
      <button 
      ...
      <table class="min-w-full bg-white">
        <thead>
          <tr class="bg-gray-100 text-gray-600 text-sm leading-normal">
            ...
            <th class="py-3 px-4 text-left">荣誉名称</th>
            <th class="py-3 px-4 text-left">年级</th>
            <th class="py-3 px-4 text-left">荣誉类型</th>
            <th class="py-3 px-4 text-left">荣誉描述</th>
            <th class="py-3 px-4 text-center">操作</th>
          </tr>
        </thead>
        <tbody class="text-gray-600 text-sm">
          <tr 
            v-for="record in honors" 
            :key="record.id"
    ...
    <!-- 空数据状态 -->
    <div v-if="honors.length === 0" class="py-10 text-center text-gray-500">
    ...
    <div v-if="showAddModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 w-full max-w-md">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-bold text-gray-800">添加其他荣誉记录</h3>
          <button 
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">荣誉名称</label>
            <input 
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">荣誉类型</label>
            <input 
              v-model="newRecord.type"
              type="text" 
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="请输入荣誉类型（如：体育之星、艺术之星等）"
            />
          </div>
    ...
    <div v-if="editingRecord" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 w-full max-w-md">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-bold text-gray-800">编辑其他荣誉记录</h3>
          <button 
            class="text-gray-500 hover:text-gray-700"
